...
you_password = "123456"  # Желаемый пароль
...
class PasswordAuth:
    ...
    def load_settings(self):
        """Loads settings, including the password, from the settings file."""
        global you_password
        try:
            with open(SETTINGS_FILE, "r") as f:
                settings = json.load(f)
                you_password = settings.get("password", you_password)  # Load the password
        except FileNotFoundError:
            # If the file doesn't exist, use default password and create the file
            self.save_settings()
        except json.JSONDecodeError:
            messagebox.showerror("Ошибка", "Ошибка при чтении файла настроек. Использован пароль по умолчанию.")

    def save_settings(self):
        """Saves the password to the settings file."""
        settings = {"password": you_password}
        try:
...
class EmotionTrackerGUI:
    ...
    def create_settings_frame(self):
        ...
        self.settings_frame.pack(pady=20, padx=60, fill="both", expand=True)

        customtkinter.CTkLabel(master=self.settings_frame, text="Настройки", font=('Arial', 30)).pack(pady=20)

        # Change password
        customtkinter.CTkLabel(master=self.settings_frame, text="Новый пароль:").pack(pady=5)
        ...
        def change_password():
            ...
            if new_password:
                ...
                messagebox.showerror("Ошибка", "Пожалуйста, введите новый пароль.")

        customtkinter.CTkButton(master=self.settings_frame, text="Изменить пароль", command=change_password).pack(
            pady=10)
    ...
    def create_main_frame(self):
        ...
        self.settings_button = customtkinter.CTkButton(master=self.main_frame, text="Настройки",
                                                       command=self.create_settings_frame, width=100)
        self.settings_button.pack(anchor="nw", padx=10, pady=10)  # Position in top left
        ...
        customtkinter.CTkLabel(output_frame, text="Введите текущий год:").grid(row=0, column=0, padx=5, pady=5,
                                                                               sticky="w")
        ...
        customtkinter.CTkLabel(output_frame, text="Введите текущий месяц:").grid(row=1, column=0, padx=5, pady=5,
                                                                                 sticky="w")
        ...
        customtkinter.CTkLabel(output_frame, text="Введите текущий день:").grid(row=2, column=0, padx=5, pady=5,
                                                                                sticky="w")
    ...
    def show_results(self):
        ...
        persons_faces_path = os.path.join(self.current_output_folder, "persons_faces")  # Use self.current_output_folder

...
def train_epoch_multimodal(epoch, data_loader, model, criterion, optimizer, opt,
    ...
    for i, (audio_inputs, visual_inputs, targets) in enumerate(data_loader):
        ...
        if i % 10 ==0:
            ...
            iter = (epoch - 1) * len(data_loader) + (i + 1)
            mlflow.log_metric('iter', iter, step=iter)
            mlflow.log_metric('train_loss', losses.avg.item(), step=iter)
            mlflow.log_metric('train_prec1', top1.avg.item(), step=iter)
            mlflow.log_metric('train_prec5', top5.avg.item(), step=iter)
            mlflow.log_metric('lr', optimizer.param_groups[0]['lr'], step=iter)   
